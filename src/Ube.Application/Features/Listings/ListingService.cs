...
public class ListingService : IListingService
{
    ...
    private static ListingResponse MapToResponse(Listing l) => new()
    {
        ...
        PrimaryImage       = l.ThumbnailUrl ?? l.Images.FirstOrDefault()?.ImageUrl,
        Images             = l.Images.Select(i => i.ImageUrl).ToList(),
        Tags               = string.IsNullOrWhiteSpace(l.Tags)
                                ? new List<string>()
                                : l.Tags.Split(',', StringSplitOptions.RemoveEmptyEntries).ToList(),
        CancellationPolicy = l.CancellationPolicy,
        CustomFieldValues  = l.CustomFieldValues.Select(c => new ListingCustomFieldValueDto
        {
            CategoryCustomFieldId = c.CategoryCustomFieldId,
            Value                 = c.Value,
        }).ToList(),
