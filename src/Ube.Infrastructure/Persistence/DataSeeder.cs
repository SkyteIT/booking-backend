...
public static class DataSeeder
{
    ...
    {
        // ── Categories ────────────────────────────────────────────────────────
        // Categories are now managed entirely through the Admin Panel.
        // Do NOT seed categories here — adding them via the UI was returning
        // 409 Conflict because the seeder had already inserted them on startup.
        // ─────────────────────────────────────────────────────────────────────

        // ── Sample Listings (only inserted if their category already exists) ──
        // These are kept so the public search page has demo data to show.
        // If you want a clean database with no sample data, delete this block.

        var categoryMap = await db.Categories
            .Where(c => c.Status != RecordStatus.Deleted)
            .ToDictionaryAsync(c => c.Name, c => c.Id);

        // Nothing to seed if no categories exist yet
        if (categoryMap.Count == 0) return;
